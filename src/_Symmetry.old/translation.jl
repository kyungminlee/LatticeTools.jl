...
function get_generators(hypercube ::HypercubicLattice)
  ...
  generators = factorize(generator_indices, initial_translation_groups, 1, 1)
  # TODO: add remaining order-1 translations
...
function is_compatible(
    fractional_momentum ::AbstractVector{R},
    identity_translation ::AbstractVector{<:Integer}
    ) where R<:Rational
  value = Rational{Int}(0)
  for (i,j) in zip(fractional_momentum, identity_translation)
    value += i * j
  end
